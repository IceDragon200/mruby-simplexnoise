...
static void AddSamples(long xi, long yi, long zi, long max_order,
                       sn_float at[3], sn_float *F,
                       sn_float(*delta)[3], unsigned long *ID);


/* The main function! */
void Worley(sn_float at[3], long max_order,
            sn_float *F, sn_float(*delta)[3], unsigned long *ID)
{
  sn_float x2, y2, z2, mx2, my2, mz2;
  sn_float new_at[3];
  long int_at[3], i;
  ...
  /* A simple way to compute the closest neighbors would be to test all
     ...
     that avoids this needless testing of distant cubes. This sn_floats the
