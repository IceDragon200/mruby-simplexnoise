 ...
 *
 * Note that making this an sn_int[] instead of a char[] might make the
...
static sn_float grad1(sn_int hash, sn_float x)
{
  sn_int h = hash & 15;
  sn_float grad = 1.0 + (h & 7);  // Gradient value 1.0, 2.0, ..., 8.0
...
static sn_float grad2(sn_int hash, sn_float x, sn_float y)
{
  sn_int h = hash & 7;      // Convert low 3 bits of hash code
  sn_float u = h < 4 ? x : y; // into 8 simple gradient directions,
  sn_float v = h < 4 ? y : x; // and compute the dot product with (x,y).
  return ((h & 1) ? -u : u) + ((h & 2) ? -2.0 * v : 2.0 * v);
}

static sn_float grad3(sn_int hash, sn_float x, sn_float y , sn_float z)
{
  sn_int h = hash & 15;     // Convert low 4 bits of hash code into 12 simple
  sn_float u = h < 8 ? x : y; // gradient directions, and compute dot product.
  sn_float v = h < 4 ? y : h == 12 || h == 14 ? x : z; // Fix repeats at h = 12 to 15
  return ((h & 1) ? -u : u) + ((h & 2) ? -v : v);
}

static sn_float grad4(sn_int hash, sn_float x, sn_float y, sn_float z, sn_float t)
{
  sn_int h = hash & 31;      // Convert low 5 bits of hash code into 32 simple
  sn_float u = h < 24 ? x : y; // gradient directions, and compute dot product.
  sn_float v = h < 16 ? y : z;
  sn_float w = h < 8 ? z : t;
...
//---------------------------------------------------------------------
/** 1D sn_float Perlin noise, SL "noise()"
 */
...
//---------------------------------------------------------------------
/** 1D sn_float Perlin periodic noise, SL "pnoise()"
 */
...
//---------------------------------------------------------------------
/** 2D sn_float Perlin noise.
 */
sn_float noise2(sn_float x, sn_float y)
{
  sn_int ix0, iy0, ix1, iy1;
  sn_float fx0, fy0, fx1, fy1;
  sn_float s, t, nx0, nx1, n0, n1;
  ix0 = FASTFLOOR(x);   // Integer part of x
...
//---------------------------------------------------------------------
/** 2D sn_float Perlin periodic noise.
 */
...
//---------------------------------------------------------------------
/** 3D sn_float Perlin noise.
 */
sn_float noise3(sn_float x, sn_float y, sn_float z)
{
  sn_int ix0, iy0, ix1, iy1, iz0, iz1;
  sn_float fx0, fy0, fz0, fx1, fy1, fz1;
  sn_float s, t, r;
  sn_float nxy0, nxy1, nx0, nx1, n0, n1;
  ix0 = FASTFLOOR(x);   // Integer part of x
  iy0 = FASTFLOOR(y);   // Integer part of y
...
//---------------------------------------------------------------------
/** 3D sn_float Perlin periodic noise.
 */
...
//---------------------------------------------------------------------
/** 4D sn_float Perlin noise.
 */

sn_float noise4(sn_float x, sn_float y, sn_float z, sn_float w)
{
  sn_int ix0, iy0, iz0, iw0, ix1, iy1, iz1, iw1;
  sn_float fx0, fy0, fz0, fw0, fx1, fy1, fz1, fw1;
  sn_float s, t, r, q;
  sn_float nxyz0, nxyz1, nxy0, nxy1, nx0, nx1, n0, n1;
  ix0 = FASTFLOOR(x);   // Integer part of x
  iy0 = FASTFLOOR(y);   // Integer part of y
...
//---------------------------------------------------------------------
/** 4D sn_float Perlin periodic noise.
 */
