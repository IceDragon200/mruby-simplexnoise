...
static void gradrot2(sn_int hash, sn_float sin_t, sn_float cos_t, sn_float *gx, sn_float *gy)
{
  sn_int h = hash & 7;
  sn_float gx0 = grad2[h][0];
  sn_float gy0 = grad2[h][1];
  *gx = cos_t * gx0 - sin_t * gy0;
  *gy = sin_t * gx0 + cos_t * gy0;
  return;
}

static void gradrot3(sn_int hash, sn_float sin_t, sn_float cos_t, sn_float *gx, sn_float *gy, sn_float *gz)
{
  sn_int h = hash & 15;
  sn_float gux = grad3u[h][0];
  sn_float guy = grad3u[h][1];
  sn_float guz = grad3u[h][2];
  sn_float gvx = grad3v[h][0];
  sn_float gvy = grad3v[h][1];
  sn_float gvz = grad3v[h][2];
...
static sn_float graddotp2(sn_float gx, sn_float gy, sn_float x, sn_float y)
{
  return gx * x + gy * y;
}

static sn_float graddotp3(sn_float gx, sn_float gy, sn_float gz, sn_float x, sn_float y, sn_float z)
{
 ...
 */
sn_float srdnoise2(sn_float x, sn_float y, sn_float angle, sn_float *dnoise_dx, sn_float *dnoise_dy)
{
  ...
  sn_float sin_t, cos_t; /* Sine and cosine for the gradient rotation angle */
  sin_t = sin(angle);
  cos_t = cos(angle);
  ...
  if ((dnoise_dx != 0) && (dnoise_dy != 0)) {
     ...
     */
    sn_float temp0 = t20 * t0 * graddotp2(gx0, gy0, x0, y0);
    *dnoise_dx = temp0 * x0;
    *dnoise_dy = temp0 * y0;
    sn_float temp1 = t21 * t1 * graddotp2(gx1, gy1, x1, y1);
    *dnoise_dx += temp1 * x1;
    *dnoise_dy += temp1 * y1;
    sn_float temp2 = t22 * t2 * graddotp2(gx2, gy2, x2, y2);
    *dnoise_dx += temp2 * x2;
    *dnoise_dy += temp2 * y2;
    *dnoise_dx *= -8.0f;
    *dnoise_dy *= -8.0f;
...
sn_float srdnoise3(sn_float x, sn_float y, sn_float z, sn_float angle,
                sn_float *dnoise_dx, sn_float *dnoise_dy, sn_float *dnoise_dz)
{
  ...
  if ((dnoise_dx != 0) && (dnoise_dy != 0) && (dnoise_dz != 0)) {
    /*  A straight, unoptimised calculation would be like:
       ...
       */
    sn_float temp0 = t20 * t0 * graddotp3(gx0, gy0, gz0, x0, y0, z0);
    *dnoise_dx = temp0 * x0;
    *dnoise_dy = temp0 * y0;
    *dnoise_dz = temp0 * z0;
    sn_float temp1 = t21 * t1 * graddotp3(gx1, gy1, gz1, x1, y1, z1);
    *dnoise_dx += temp1 * x1;
    *dnoise_dy += temp1 * y1;
    *dnoise_dz += temp1 * z1;
    sn_float temp2 = t22 * t2 * graddotp3(gx2, gy2, gz2, x2, y2, z2);
    *dnoise_dx += temp2 * x2;
    *dnoise_dy += temp2 * y2;
    *dnoise_dz += temp2 * z2;
    sn_float temp3 = t23 * t3 * graddotp3(gx3, gy3, gz3, x3, y3, z3);
    *dnoise_dx += temp3 * x3;
    *dnoise_dy += temp3 * y3;
    *dnoise_dz += temp3 * z3;
    *dnoise_dx *= -8.0f;
